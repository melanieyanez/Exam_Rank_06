...
void broadcast(int fd)
{
	...
	{
		if (FD_ISSET(i, &readyWrite) && i != fd) //attention readyWrite
...
int main(int argc, char **argv)
{
	...
	while (42)
	{
		...
		if (select(max_fd + 1, &readyRead, &readyWrite, NULL, NULL) < 0) //attention max_fd + 1, retenir select
			continue;
		for (int fd = 0; fd <= max_fd; fd++)
		{
			if (FD_ISSET(fd, &readyRead)) //attention readyRead
			{
				if (fd == server_socket) //attention tout avec client_socket
				{
				...
				else
				{
					...
					int nbytes = recv(fd, bufRead, sizeof(bufRead), 0); //retenir recv
					if (nbytes <= 0) //se deco
					{
